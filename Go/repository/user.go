...
func ListCreditLogs(q model.Query) ([]model.CreditLog, int64, error) {
	...
	if keyword := strings.TrimSpace(q.Keyword); keyword != "" {
		like := "%" + keyword + "%"
		tx = tx.Where("credit_logs.user_id LIKE ? OR type LIKE ? OR remark LIKE ? OR related_id LIKE ?", like, like, like, like)
	}
	var total int64
	if err := tx.Count(&total).Error; err != nil {
		return nil, 0, err
...
// ListCreditLogsByUser 查询指定用户的算力点流水。
func ListCreditLogsByUser(userID string, q model.Query) ([]model.CreditLog, int64, error) {
	...
	q.Normalize()
	tx := db.Model(&model.CreditLog{}).Where("user_id = ?", strings.TrimSpace(userID))
	if keyword := strings.TrimSpace(q.Keyword); keyword != "" {
		like := "%" + keyword + "%"
