...
import { fetchAdminSettings, measureAdminStorageProvider, saveAdminSettings, type AdminModelChannel, type AdminModelCost, type AdminSettings, type AdminStorageProvider } from "@/services/api/admin";
...
export default function AdminSettingsPage() {
    ...
    const watchedChannels = (Form.useWatch(["private", "channels"], form) || []) as AdminModelChannel[];
    const channelModels = useMemo(() => collectChannelModels(watchedChannels), [watchedChannels]);
                    ...
                    {activeTab === "public" ? (
                        activeMode === "visual" ? (
                            <Form form={form} layout="vertical" initialValues={emptySettings} requiredMark={false}>
                                <Row gutter={16}>
                                    <Col span={24}>
                                        <Form.Item name={["public", "modelChannel", "availableModels"]} label="系统可用模型" extra="可选项来自「模型管理」中各启用模型配置的模型。最终开放哪些模型由这里勾选决定">
