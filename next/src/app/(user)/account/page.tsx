...
import { Button, Card, Col, Descriptions, Divider, Empty, Form, Input, Modal, Pagination, Row, Statistic, Table, Tabs, Tag, Tooltip, Typography } from "antd";
import { ArrowRight, Check, Copy, LogOut, Pencil, Sparkles, Video } from "lucide-react";
...
export default function AccountPage() {
    ...
    const recordColSpan = recordCards.length > 1 ? 12 : 24;
    ...
    const creditColumns = [
        ...
        { title: <Tooltip title="模型消费与对应失败返还使用同一个生成任务 ID；后台调整使用独立操作 ID，不是账号 ID。"><span className="cursor-help border-b border-dashed border-border">关联 ID</span></Tooltip>, dataIndex: "relatedId", width: 170, ellipsis: true, render: (value: string) => value ? <Typography.Text copyable={{ text: value }} ellipsis>{value}</Typography.Text> : "-" },
    ];
    ...
    return (
        <main className="h-full overflow-y-auto bg-background text-foreground">
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-8 lg:py-10">
                ...
                <Card variant="borderless" className="!bg-card" title={<div><span className="font-semibold">算力流水</span><span className="ml-2 text-xs font-normal text-muted-foreground">共 {totalLogs} 条</span></div>}><Table<CreditLog> rowKey="id" columns={creditColumns} dataSource={logs} loading={isLoading} pagination={false} scroll={{ x: 760 }} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无算力流水" /> }} />{totalLogs ? <div className="mt-5 flex justify-end"><Pagination current={page} pageSize={pageSize} total={totalLogs} showSizeChanger={false} onChange={changePage} /></div> : null}<p className="mt-3 text-xs text-muted-foreground">关联 ID 用于追踪生成任务或后台操作；同一任务的模型消费与失败返还会显示相同 ID。</p></Card>
                <section><div className="mb-3"><h2 className="text-lg font-semibold">创作记录</h2></div>{recordCards.length ? <Row gutter={[12, 12]}>{recordCards.map((card) => <Col key={card.title} xs={24} sm={12} lg={recordColSpan}><RecordCard {...card} /> </Col>)}</Row> : null}{canvasGroups.length ? <div className={recordCards.length ? "mt-4" : ""}><Tabs activeKey={activeCanvasId || canvasGroups[0].projectId} onChange={(value) => setActiveCanvasId(value)} items={canvasGroups.map((group) => ({ key: group.projectId, label: <span className="inline-flex max-w-full items-center gap-2"><span className="max-w-[12rem] truncate" title={group.projectTitle}>{group.projectTitle || "未命名画布"}</span><span className="text-xs opacity-60">{group.records.length}</span></span>, children: <div className="rounded-xl border border-border bg-card p-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{group.records.map((record) => <CanvasMediaCard key={record.id} record={record} />)}</div></div> }))} /></div> : <div className="mt-4 rounded-xl border border-dashed border-border bg-card py-10"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="画布中还没有生成的图片或视频" /></div>}</section>
            </div>
