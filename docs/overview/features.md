...
图片生成支持「比例」和「分辨率档位」两个独立维度（如智能比例 + 2K/4K 组合），存量像素尺寸配置打开时自动迁移。代理请求的档位翻译由后端按各模型的渠道适配参数（档位映射：quality/resolution/size + 比例处理方式）在管理后台配置，未配置时按 OpenAI 标准协议折算像素

视频请求归一化同样由后台配置驱动：管理后台「模型定价」页可为每个视频模型配置渠道适配参数（比例字段名、分辨率表达、quality、参考图/视频/音频字段名与组装模式——如纯 URL 数组、首尾帧双字段、仅首帧、多帧序列、图+角色配对等），未配置时走通用协议默认；聚合渠道接入新视频模型时后台配置即可，无需改代码
...
- 新增、编辑、删除提示词
- 按分类和标签筛选
- JSON 描述文件 + 封面图片批量导入，封面文件名与 `coverUrl` 一致时自动替换为上传地址

提示词内容不再从 GitHub 远程同步，全部由管理后台手动维护和批量导入。
