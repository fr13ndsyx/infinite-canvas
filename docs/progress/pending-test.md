...
- 新增受登录保护的 `/account` 个人中心页面，展示账号资料、角色、注册/登录时间、邀请码、算力余额与最近 30 天变动汇总；个人中心不展示头像或头像上传入口。
- 新增当前用户算力流水分页查询；流水按用户身份强制过滤，不接受前端传入其他用户 ID。
- 支持编辑昵称和邮箱，保存后同步顶栏账户名称；修改密码使用独立弹窗，并校验旧密码。
- 增加图片历史、视频历史快捷入口（画布中无对应生成记录时才显示），并按画布 Tab 分组展示画布节点中已生成的图片与视频预览；个人中心保留退出登录操作。
- 顶栏账户下拉中的“个人中心”已接通 `/account`，未登录访问会跳转登录页。
- 个人中心右上角和账户下拉菜单均提供“修改密码”，点击后直接打开独立的修改密码弹窗，不进入配置弹窗。
- 算力概览卡片使用主题语义色，浅色/深色模式下余额卡与统计文字保持可读。
- 账户菜单各项统一使用主题语义文字颜色，并在相邻项目之间增加横线；算力流水补充“关联 ID”说明，展示生成任务或后台操作的业务追踪编号，未关联时显示“-”。
- 算力流水已接通关联 ID：图片、音频、视频任务使用生成任务 ID，普通模型请求与工作流 Agent 使用请求追踪 ID；模型消费和对应失败返还复用同一 ID，后台手动调整生成独立操作 ID。
- 画布已有图片或视频记录时，分别隐藏对应的“图片历史”或“视频历史”快捷卡片，避免重复入口。
- 创作记录区移除“我的素材”“我的画布”快捷卡片；画布切换由切换按钮改为 Tab 形式，Tab 标题为画布名称与记录数，内容区域复用同一面板，面板内不再提供点击跳转到对应画布的入口。
- 精简个人中心提示文案：移除页头 ACCOUNT 角标与副标题、账号卡 @用户名、创作记录副标题、媒体面板内的记录数文字；修复昵称/用户名与“管理员”标签的垂直对齐（消除标题默认外边距导致标签偏低的问题）。
- 后台算力点日志“用户 ID”列改为“用户”列，显示“昵称（用户 ID）”格式：昵称优先取昵称、为空回退用户名，ID 保留一键复制，超长省略号并悬停显示完整内容。

### 验证步骤

1. 登录普通用户，打开顶栏账户菜单进入“个人中心”，确认页面信息与当前账号一致，浅色/深色主题下卡片、表格和弹窗均可读。
2. 修改昵称或邮箱，保存后刷新页面和顶栏，确认资料保持；输入无效邮箱时确认表单拦截，确认页面没有头像上传入口。
3. 查看算力余额、最近 30 天汇总和流水分页，确认只显示当前用户记录，变动正负颜色正确。
4. 从个人中心或顶栏账户菜单打开独立的“修改密码”弹窗，使用旧密码修改新密码，确认旧密码错误或两次新密码不一致时无法提交，成功后可用新密码登录。
5. 点击图片历史或视频历史快捷入口（画布无对应记录时才显示）和退出登录，确认跳转/登录态行为正常；确认个人中心能看到画布中生成的图片、视频卡片；未登录直接访问 `/account` 应回到登录页。
6. 在个人中心和顶栏账户菜单分别点击“修改密码”，确认打开的是独立密码弹窗；切换浅色/深色主题确认账户菜单与算力概览文字和背景有足够对比度，菜单相邻项目之间均有横线；确认关联 ID 的提示与空值展示正确；创建图片/视频画布记录后确认对应历史快捷卡片隐藏；在创作记录中确认可切换画布，点击画布列表项可进入画布，点击单条媒体记录不会跳转。
7. 使用云端渠道分别创建图片、音频和视频任务，确认模型消费流水的关联 ID 与返回的任务 ID 一致；触发一次可返还的模型调用失败，确认消费与失败返还两条流水的关联 ID 相同；后台调整算力后确认流水显示独立操作 ID。
8. 打开管理后台“算力点日志”，确认“用户”列显示“昵称（用户 ID）”，复制按钮复制的是纯用户 ID；把昵称清空为只有用户名的账号，确认列显示用户名兜底；悬停省略行显示完整“昵称（用户 ID）”。
9. 打开个人中心“创作记录”，确认不再显示“我的素材”“我的画布”卡片；画布切换为 Tab 形式且 Tab 标题是画布名称与记录数，切换 Tab 后内容面板更新，面板内没有任何点击跳转到画布的入口。
10. 打开个人中心各区块，确认页头只有“个人中心”标题、账号卡只有“昵称 + 角色标签”、创作记录无副标题、媒体面板无记录数文字；浅色/深色主题下确认昵称与“管理员”标签垂直居中对齐。

## antd Space direction 弃用警告修复

### 可测试变更

- antd 6 中 Space 组件的 `direction` 属性已更名为 `orientation`；已替换模型定价（2 处）、算力点日志、渠道管理页面共 4 处，控制台不再出现弃用警告。

### 验证步骤

1. 打开管理后台模型定价、算力点日志、渠道管理页面，确认浏览器控制台无 `[antd: Space] direction is deprecated` 警告。

## 技能模块（上线运营需求 4）

### 可测试变更

- 新增后端 `skills` 技能表与公开接口 `GET /api/skills?nodeType=`（仅返回已上架技能，支持 text / image / video 过滤），管理后台接口 `/api/admin/skills` CRUD。
- 后端首次启动且技能表为空时，自动写入首批预置技能：文本（翻译、文本扩写）、图片（16 宫格连贯分镜、电影级光影矫正）、视频（运镜-轨道右移、运镜-环绕拍摄）。
- 画布节点底部助手栏在提示词库按钮旁新增"技能"按钮（音频节点不显示），点击弹出按当前节点类型过滤的技能面板（名称 + 一句话说明）。
- 点击技能后，输入框展示带 SVG 图标、技能名称和删除按钮的技能标签，模板内容不直接展开；技能模板仍会参与生成，点击删除或按退格可移除整个标签。
- 技能浮层改为固定定位 Portal，跟随画布缩放、滚动和窗口变化重新定位，并复用视频能力弹窗的箭头与主题样式；选择技能后仍显示输入提示文案。
- 画布普通图片上传框改为透明背景的悬浮缩放交互，与首尾帧上传框保持一致。
- 画布节点弹窗统一使用浅色未选中 `#ffffff` / 选中 `#e7e5df`，深色未选中 `#1f1f1f` / 选中 `#3a3631`；上传框悬浮时会回正，技能标签下方的输入提示不再被遮挡。
- 技能标签输入在清空补充文字后会把光标保持在标签之后；视频比例选项按弹窗宽度自适应排列，视频能力的“全能参考/首尾帧”选项补齐未选中底色。
- 视频能力、视频/图片的比例与分辨率选项统一由单个带底色和阴影的分组容器包裹，按钮本身只保留选中态填充。
- 修复技能标签后删除补充文字时光标回跳到标签前的问题；深色模式下分组容器增加高对比阴影和内描边，边界更清晰。
- 首尾帧上传框增加悬浮选中态黑色边框，未选中时保持无边框。
- 首尾帧与普通上传框共用主题边框：浅色未选中浅灰/选中黑色，深色未选中浅白灰/选中白色。
- 管理后台新增"技能管理"页（内容库分组）：支持新增/编辑/删除技能、节点类型选择、排序、上下架开关与封面 URL；列表实时反映上架状态。
- 视频节点通用能力弹窗中的运动控制入口保持现状，运动控制后续由视频技能模块承接。

### 验证步骤

1. 启动后端，确认启动日志无 seed 报错；打开画布新建文本/图片/视频节点，点击助手栏"技能"按钮，确认面板按节点类型显示对应预置技能。
2. 点击某个技能，确认输入框出现技能标签而不是展开模板；输入自定义补充内容后生成，确认技能模板与补充内容一起生效；点击标签删除按钮或将光标置于标签后按退格，确认整个标签被移除。
3. 进入管理后台"技能管理"，新增一条技能并上架，回到画布对应节点确认新技能出现；下架后确认画布面板不再显示。
4. 编辑技能名称/说明/排序并保存，确认画布面板展示同步更新；删除技能后确认列表与画布面板同步移除。
5. 未登录状态打开画布，确认技能按钮与面板可正常使用（公开接口无需登录）。
...
- 登录后的顶栏账户菜单保留“配置与偏好”入口，个人中心页面不重复放置配置入口；配置入口和弹窗内容不使用 SVG 图标。
...
3. 登录前后确认顶栏主题切换按钮都在同一位置；登录后从账户菜单分别打开“配置与偏好”和“修改密码”，确认两个弹窗相互独立且功能正常。
...
- 后端管理后台用户搜索 SQL 移除 `linux_do_id` 条件：模型字段已删但 SQL 残留，导致管理后台搜索用户时报「字段 linux_do_id 不存在」（PG 日志实际报错）
- 管理后台用户搜索框占位文案移除「或 Linux.do ID」
- 文档 `backend-database.md`、`system-settings.md` 移除残留的 `linux_do_id` 字段与 `auth.linuxDo` 配置说明
...
5. 保存系统配置，确认不报错
6. 在 `/admin/users` 搜索框输入关键词搜索，确认正常返回结果、不再报「字段 linux_do_id 不存在」
