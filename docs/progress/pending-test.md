...
- 增加图片历史、视频历史、我的素材、我的画布快捷入口，并按画布分组展示画布节点中已生成的图片与视频预览；只有画布列表项可跳转，单条媒体记录不可单独点击；个人中心保留退出登录操作。
- 顶栏账户下拉中的“个人中心”已接通 `/account`，未登录访问会跳转登录页。
- 个人中心右上角和账户下拉菜单均提供“修改密码”，点击后直接打开独立的修改密码弹窗，不进入配置弹窗。
- 算力概览卡片使用主题语义色，浅色/深色模式下余额卡与统计文字保持可读。
- 账户菜单各项统一使用主题语义文字颜色，并在相邻项目之间增加横线；算力流水补充“关联 ID”说明，明确该字段是预留业务追踪编号，未关联时显示“-”。
...
5. 点击四个创作记录入口和退出登录，确认跳转/登录态行为正常；确认个人中心能看到画布中生成的图片、视频卡片并可回到对应画布；未登录直接访问 `/account` 应回到登录页。
