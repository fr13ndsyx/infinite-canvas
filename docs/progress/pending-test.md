...
- 新增站内版本接口 `GET /api/app-version`，返回构建时注入的当前版本号与最近 8 个版本的更新日志，响应禁用缓存。
- 版本检测数据源由 GitHub raw 改为站内接口，不再依赖境外网络；检测时机为页面挂载、每 10 分钟轮询、标签页切回前台。
- 顶栏恢复版本号入口：显示当前版本号，检测到新版本时右上角显示绿点，点击打开更新日志弹窗（含"检查更新"按钮与历史版本时间线）。
- 新增右下角非阻塞更新提示卡片：展示新版本号、当前 → 最新版本与最新版本前 5 条更新要点，含"更新将刷新页面，请先保存当前编辑内容"提示。
- 提示卡片操作：「立即更新」刷新页面加载新资源；「稍后」在当前标签页会话内不再提示同一版本。
- 管理后台顶部同样挂载更新提示卡片。

### 验证步骤

1. 请求 `GET /api/app-version`，确认返回 `{ version, releases }` 且 `releases` 最多 8 项，响应头含 `Cache-Control: no-store`。
2. 确认顶栏显示版本号，点击打开更新日志弹窗，时间线正常渲染；点击"检查更新"可刷新。
3. 手动提升根目录 `VERSION` 并重启前端，页面在 10 分钟内或切回前台后弹出右下角更新提示，内容取自 CHANGELOG 最新版本。
4. 点击「立即更新」页面刷新；点击「稍后」卡片关闭且本次会话不再弹出，新开标签页后重新提示。
5. 进入画布页（顶栏隐藏）与管理后台，确认更新提示卡片均能正常弹出，浅色/深色主题下文字清晰。
