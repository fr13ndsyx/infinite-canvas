...
- 状态：待实施
- 方案文档：[admin-nav-restructure.md](./admin-nav-restructure.md)
- 目标：按管理员工作流重组后台导航（用户与资费 / 模型服务 / 内容库 / 系统四组），隐藏"公开/私有配置"实现概念；原系统设置页拆分为「开放与定价」「存储设置」「系统偏好」「高级配置」四个新页面，promptSync 进提示词来源页、aiLog 进 AI 日志页
- 改动范围：前端 4 个新页面 + layout 菜单 + settings 页改重定向 + 2 个功能页加设置卡片；后端零改动
- 关键点：
  - 保留用户管理、提示词来源、提示词管理、素材库、模型管理五个页面不动
  - 沿用全量 settings 读写模式，不拆分域接口
  - 开放与定价页含"未定价模型"醒目警告，防商用漏定价
  - `/admin/settings` 重定向到 `/admin/model-pricing`
